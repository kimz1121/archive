...
class bmp_editor
{
	...
	char* edit_data_reverse_buff = NULL;
	...
	bmp_editor(const char *f_in_name)
	{
		...
		edit_head_buff = new char[54];
		edit_data_buff = new char[bitmap_INFO_HEADER.biWidth*bitmap_INFO_HEADER.biHeight*(bitmap_INFO_HEADER.biBitCount/8)];
		edit_data_reverse_buff = new char[bitmap_INFO_HEADER.biWidth*bitmap_INFO_HEADER.biHeight*(bitmap_INFO_HEADER.biBitCount/8)];
	...
	void read_header_bmp()// 헤더를 수동으로 읽어 구조체가 차지하는 메모리를 낭비하지 않는 방법도 나중에 시도해 보자
	{
...
int main()
{
	bmp_editor editor("sample_iw_9_300_300_size_bmp.bmp");
