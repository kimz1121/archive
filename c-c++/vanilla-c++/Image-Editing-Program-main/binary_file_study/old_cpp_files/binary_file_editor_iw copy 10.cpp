...
class bmp_editor
{
	...
	bmp_editor(const char *f_in_name)
	{
		...
		edit_refined_data_buff = new char[bitmap_INFO_HEADER.biWidth*bitmap_INFO_HEADER.biHeight*(bitmap_INFO_HEADER.biBitCount/8)];
		cout <<bitmap_INFO_HEADER.biWidth*bitmap_INFO_HEADER.biHeight*(bitmap_INFO_HEADER.biBitCount/8)<< endl;
	...
	void edit_head_load()//헤더를 수동으로 읽어 구조체가 차지하는 메모리를 낭비하지 않는 방법도 나중에 시도해 보자
	{
		...
		memmove(edit_head_buff, buff_ptr, 50);//왜 에러 일까남... 디버그 돌리고 싶당...ㅠㅠ//편집하다보니 new 키워드 보다 함수의 실행 시점이 늦어진 것이 문제였음.
	...
	{	
		...
		while(i < length_of_col)
		{
			...
			memmove(reverse_data_buff+((length_of_col-i-1)*4*length_of_row), package_data_buff+(i*4*length_of_row), length_of_row);
			i = i+1;
		}
		cout<<"4"<<endl;
	...
	void edit_image_load()
	{
		...
		while(i < length_of_col)
		{
			memmove(edit_refined_data_buff+(i*byte_for_pixel*width), reverse_data_buff+(i*4*length_of_row), width);
			i = i+1;
		}
	}

	void edit_image_store()
	{
		int height = bitmap_INFO_HEADER.biHeight;
		int width = bitmap_INFO_HEADER.biWidth;
		int byte_for_pixel = (bitmap_INFO_HEADER.biBitCount/8);
		...
		while(i < length_of_col)
		{
			memmove(reverse_data_buff+(i*4*length_of_row), edit_refined_data_buff+(i*byte_for_pixel*width), width);
			i = i+1;
		}
	}
	...
	{	
		...
		memmove(package_data_buff, 0, bitmap_INFO_HEADER.biSizeImage);

		while(i < length_of_col)
		{
			memmove(package_data_buff+(i*4*length_of_row), reverse_data_buff+((length_of_col-i-1)*4*length_of_row), length_of_row);
			i = i+1;
		}
	...
	void write_data_bmp()
	{
		open_file("sample_binary_OUTPUT_2.bmp", 'O');
		f_out_image.seekp(54, ios::beg);
		f_out_image.write(rare_buff, bitmap_INFO_HEADER.biSizeImage);
	}
	...
	~bmp_editor()
	{
		...
		delete reverse_data_buff;
		delete edit_refined_data_buff;
...
int main()
{
	bmp_editor editor("sample_iw_16_11_11_size_bmp.bmp");
