...
class bmp_editor
{
	...
	void read_header_bmp()
	{
		...
		temp_f_header_struct = (((uint32_t*)&bitmap_FILE_HEADER)+(sizeof(void*)));
		buff_ptr = buff+1;
		while (n < 5)
		{
			bit_temp = 0;
			...
			temp_f_header_struct = temp_f_header_struct+(sizeof(void*));
			cout<<(void*)temp_f_header_struct<<endl;
		...
		while (n < 11)
		{
			...
			temp_I_header_struct = temp_I_header_struct+(sizeof(void*));
			buff_ptr = buff_ptr+info_head[n];
			n = n + 1;
		}
	}
