...
int main()
{
    viewer V("image_viewer", 1000, 600); //이름&크기.
    ...
    editor.edit_data_load(); //원본 데이터 불러오기.(램에서 데이터 카피_편집용)
    editor.test_draw_1(); //화살표(보라색)
    V.image_safty_load(editor.image_width, editor.image_height, editor.edit_refined_data_copy_buff);
    V.screen_float();
    editor.save_as("sample_binary_OUTPUT_1.bmp"); //그림 저장
    ...
    editor.test_line(1, 1, 3, 2, 1, 0, 0xff, 0);
    V.image_safty_load(editor.image_width, editor.image_height, editor.edit_refined_data_copy_buff);
    V.screen_float();
    editor.save_as("sample_binary_OUTPUT_3.bmp");
