...
struct layer
{
    ...
    */
    ...
    void clear()
    {
        cout<<"member_width :: "<<endl;
        cout<<member_width<<endl;
        cout<<"member_height :: "<<endl;
        cout<<member_height<<endl;
        for(int i = 0; i < member_width; i++)
        {
            for(int j = 0; j < member_height; j++)
...
class bmp_editor
{
    ...
    */
    ...
    void test_line(int pos_x, int pos_y, int pos_x1, int pos_y1, bool draw, uint8_t R, uint8_t G, uint8_t B)
    {
        ...
        {   
            (*layer_arr[0]).pixel(x, gradient*(x - pos_x) + pos_y, draw, R, G, B); //점 그리기
...
class bmp_editor
{
	...
	void test_draw_0()
	{
		...
		(*layer_arr[1]).store(edit_refined_data_copy_buff, bitmap_INFO_HEADER.biWidth, bitmap_INFO_HEADER.biHeight);
		(*layer_arr[0]).store(edit_refined_data_copy_buff, bitmap_INFO_HEADER.biWidth, bitmap_INFO_HEADER.biHeight); 
	}
...
int main()
{
    bmp_editor editor("sample_iw_19_1000_1000_size_bmp.bmp"); //
    editor.area_set_total_editor(1); //영역 선택(전체)
    editor.edit_data_load();
    editor.test_line(100, 100, 900, 900, 1, 0xa0, 0x00, 0xff);
    editor.save_as("sample_line_OUTPUT_0.bmp");

    editor.edit_data_load();
    editor.test_draw_1();
    editor.save_as("sample_OUTPUT_1.bmp");

    // cout<<"0"<<endl;
    // viewer V("image_viewer", 1000, 1000); //이름&크기.
    // cout<<"1"<<endl;
    // bmp_editor editor("sample_iw_19_1000_1000_size_bmp.bmp"); //
    // cout<<"2"<<endl;
    // editor.area_set_total_editor(1); //영역 선택(전체)
    ...
    // // editor.test_line(100, 100, 900, 900, 1, 0xa0, 0x00, 0xff);
    // cout<<"4"<<endl;
    // V.image_safty_load(editor.image_width, editor.image_height, editor.edit_refined_data_copy_buff);
    // V.screen_float();
    // editor.save_as("sample_line_OUTPUT_0.bmp");
...
//  editor.test_draw_0();
//  editor.save_as("sample_binary_OUTPUT_0.bmp");
...
//  editor.edit_data_load();
//  editor.test_draw_2();
//  editor.save_as("sample_binary_OUTPUT_2.bmp");

    
//  editor.edit_data_load();
//  editor.test_draw_3();
//  editor.save_as("sample_binary_OUTPUT_3.bmp");
...
//  cout <<(editor.image_width)<< endl;
//  cout <<(editor.image_height)<< endl;
...
//  cout<<(void*)&(editor.image_width)<<endl;
//  cout<<(void*)&(editor.image_height)<<endl;
