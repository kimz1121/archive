...
char bit_analysis(int bit_old = 0b000, int bit_new = 0b000) //it return 0, 2, 3, or e. 0 means it isn't moved, 2 means forward, 3 means backward, e means error.
{
	...
	char discriminant =
		((find_coupled_nuber(bit_old) ^ bit_u_new) && (1 ^ (bit_u_new ^ bit_u_old))) * ((find_coupled_nuber(bit_old) ^ bit_v_old) * (2) + (find_coupled_nuber(bit_old) ^ bit_w_old) * (3)) + ((find_coupled_nuber(bit_old) ^ bit_v_new) && (1 ^ (bit_v_new ^ bit_v_old))) * ((find_coupled_nuber(bit_old) ^ bit_w_old) * (2) + (find_coupled_nuber(bit_old) ^ bit_u_old) * (3)) + ((find_coupled_nuber(bit_old) ^ bit_w_new) && (1 ^ (bit_w_new ^ bit_w_old))) * ((find_coupled_nuber(bit_old) ^ bit_u_old) * (2) + (find_coupled_nuber(bit_old) ^ bit_v_old) * (3))
...
void degree_to_step(struct motor_list *robot_arm_address, int number_of_motors)
{
	...
	robot_arm_1.m1_stepper.destination_step = (long)((robot_arm_1.m1_stepper.destination_degree / (2 * M_PI)) * robot_arm_1.m1_stepper.step_per_revolution_presison);
	robot_arm_1.m2_stepper.destination_step = (long)((robot_arm_1.m2_stepper.destination_degree / (2 * M_PI)) * robot_arm_1.m2_stepper.step_per_revolution_presison);
	robot_arm_1.m3_stepper.destination_step = (long)((robot_arm_1.m3_stepper.destination_degree / (2 * M_PI)) * robot_arm_1.m3_stepper.step_per_revolution_presison);
	robot_arm_1.m4_stepper.destination_step = (long)((robot_arm_1.m4_stepper.destination_degree / (2 * M_PI)) * robot_arm_1.m4_stepper.step_per_revolution_presison);
...
void position_calc_for_each_joint(struct motor_list *robot_arm_address, double position[][3])
{
	// struct motor_mathod *motor_Ptr = (struct motor_mathod *)robot_arm_address;
	...
	position[2][0] = leg_2 * cos(deg_1) * cos(M_PI_2 - deg_2);
	position[2][1] = leg_2 * sin(deg_1) * cos(M_PI_2 - deg_2);
	position[2][2] = leg_1 + leg_2 * sin(M_PI_2 - deg_2);

	position[3][0] = leg_2 * cos(deg_1) * cos(M_PI_2 - deg_2) + leg_3 * cos(deg_1) * cos(M_PI_2 - (deg_2 + deg_3));
	position[3][1] = leg_2 * sin(deg_1) * cos(M_PI_2 - deg_2) + leg_3 * sin(deg_1) * cos(M_PI_2 - (deg_2 + deg_3));
	position[3][2] = leg_1 + leg_2 * sin(M_PI_2 - deg_2) + leg_3 * sin(M_PI_2 - (deg_2 + deg_3));

	position[4][0] = leg_2 * cos(deg_1) * cos(M_PI_2 - deg_2) + leg_3 * cos(deg_1) * cos(M_PI_2 - (deg_2 + deg_3)) + leg_4 * cos(deg_1) * cos(M_PI_2 - (deg_2 + deg_3 + deg_4));
	position[4][1] = leg_2 * sin(deg_1) * cos(M_PI_2 - deg_2) + leg_3 * sin(deg_1) * cos(M_PI_2 - (deg_2 + deg_3)) + leg_4 * cos(deg_1) * cos(M_PI_2 - (deg_2 + deg_3 + deg_4));
	position[4][2] = leg_1 + leg_2 * sin(M_PI_2 - deg_2) + leg_3 * sin(M_PI_2 - (deg_2 + deg_3)) + leg_4 * sin(M_PI_2 - (deg_2 + deg_3 + deg_4));
}
...
void virtual_driving() //나중에는 이상적 로봇 위치와 모터의 각도 해상도를 고려한 모델간의 변위를 측정 해 볼 수도 있을듯
{
	...
	*/
	while (rectangular_coordinate_macroscopic_compliment_checking(&robot_arm_1, &func_gripper) == 'n')
	{
		...
		position_calc_for_each_joint(&robot_arm_1, pos);
		robot_arm_1.stream_of_time = robot_arm_1.stream_of_time + 1;
	}
}
...
int main(void)
{
	...
	*/
	...
	//PORTB = 0b11111111;
	//<시작>m1 초기값 설정=========================================
	robot_arm_1.m1_stepper.motor_type = 'v';
	robot_arm_1.m1_stepper.motor_driving_mode = 'n';
	robot_arm_1.m1_stepper.step_per_revolution_presison = 100; //1435*3*5
	...
	//<시작>m2 초기값 설정=========================================
	robot_arm_1.m2_stepper.motor_type = 'v';
	robot_arm_1.m2_stepper.motor_driving_mode = 'n';
	robot_arm_1.m2_stepper.step_per_revolution_presison = 100; //1435*3*3*5
	...
	//<끝>m2 초기값 설정=========================================
	//<시작>m3 초기값 설정=========================================
	robot_arm_1.m3_stepper.motor_type = 'v';
	robot_arm_1.m3_stepper.motor_driving_mode = 'n';
	robot_arm_1.m3_stepper.step_per_revolution_presison = 100; //1435*3*3
	...
	//<끝>m3 초기값 설정=========================================
	//<시작>m4 초기값 설정=========================================
	robot_arm_1.m4_stepper.motor_type = 'v';
	...
	//<끝>m4 초기값 설정=========================================
	//<시작>m5 초기값 설정=========================================//모터 타입이 기본값인 V 로 선언 되어 있음!!
	...
	*/
	...
	motor_combo_calibration(&robot_arm_1, &func_gripper, 36, 0, 10, M_PI_2, 255, 5); //last argument is number_of_motors
	//=============================
	///*

	combine_ordering(&robot_arm_1, &func_gripper, 40, 10, 15, M_PI_2, 255, 100);
