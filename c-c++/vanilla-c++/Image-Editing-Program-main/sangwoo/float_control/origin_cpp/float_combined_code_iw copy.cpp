...
class bmp_editor
{
	...
	bmp_editor(const char *f_in_name)
	{
		...
		pure_size_of_data = image_width*image_height*(bitmap_INFO_HEADER.biBitCount/8);
		...
		layer_arr[0] = new layer(image_width, image_height, bitmap_INFO_HEADER.biBitCount/8);
	...
	void edit_data_store()//이미지에 덭 그리고 새롭게 그려넣은 레이어를 화면으로 출력하기 성공 예제 크기 필요 있음.
	{
		int height = image_height;
		int width = image_width;
<<<<<<< HEAD
...
=======
>>>>>>> 이미지에 덭 그리고 새롭게 그려넣은 레이어를 화면으로 출력하기 성공 예제 크기 필요 있음.
		int byte_for_pixel = (bitmap_INFO_HEADER.biBitCount/8);
