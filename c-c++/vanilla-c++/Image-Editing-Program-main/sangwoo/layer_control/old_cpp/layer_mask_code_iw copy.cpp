	...
	*/
	...
	void store(char* refined_data_buff, int buff_width, int buff_height)
	{
		for (int y = 0; y < member_width; y++)
		{
			...
			{
				if(dptr_mask_shape[x][member_height-(y+1)] && dptr_mask_area[x][member_height-(y+1)])
				{
					*(refined_data_buff+(member_byte_for_pixel*x)+(member_byte_for_pixel*buff_width*y)+0) = tptr_data_coulor[x][member_height-(y+1)][0];
					*(refined_data_buff+(member_byte_for_pixel*x)+(member_byte_for_pixel*buff_width*y)+member_byte_for_pixel/3) = tptr_data_coulor[x][member_height-(y+1)][1];
					*(refined_data_buff+(member_byte_for_pixel*x)+(member_byte_for_pixel*buff_width*y)+(member_byte_for_pixel/3)*2) = tptr_data_coulor[x][member_height-(y+1)][2];
				}
...
int main()
{
	...
	layer layer_1(20, 20, 3);
	layer_1.clear();
	layer_1.area(0, 0, 19, 19);
	
	layer_1.pixel(0, 0, 1, 0xff, 0xff, 0);
	layer_1.pixel(0, 1, 1, 0xff, 0xff, 0);
	layer_1.pixel(1, 0, 1, 0xff, 0xff, 0);
	layer_1.pixel(19, 19, 1, 0xff, 0xff, 0);
	layer_1.store(editor.edit_refined_data_buff, 20, 20);
