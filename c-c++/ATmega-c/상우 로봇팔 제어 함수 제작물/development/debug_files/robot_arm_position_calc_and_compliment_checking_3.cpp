...
struct motor_mathod
{
	...
	int step_per_revolution_presison = 1000;//임시값 100. 
...
{
	//printf("check_3 %f", ((*robot_arm_address).stream_of_time / (*robot_arm_address).time_out));
...
void ordering_trans_form(struct motor_list* robot_arm_address, struct function_part* function_name_address)
{
	...
	printf("%f, %f, %f, %f, %f", robot_arm_1.m1_stepper.destination_degree, robot_arm_1.m2_bldc.destination_degree, robot_arm_1.m3_stepper.destination_degree, robot_arm_1.m4_stepper.destination_degree, robot_arm_1.m5_servo_gripper.destination_degree);
...
int main()
{
	...
	motor_combo_calibration(&robot_arm_1, &func_gripper, 59, 0, 27, M_PI, 255, 5);//last argument is number_of_motors
	printf("\n현재 step %d, %d, %d, %d, %d\n\n", robot_arm_1.m1_stepper.current_step, robot_arm_1.m2_bldc.current_step, robot_arm_1.m3_stepper.current_step, robot_arm_1.m4_stepper.current_step, robot_arm_1.m5_servo_gripper.current_step);
...
//	reset_trans_form(&robot_arm_1, &func_gripper); 
	...
	while(1)
	{
		combine_ordering(&robot_arm_1, &func_gripper, 15, 15, 15, M_PI, 255, 1000);
		printf("b");
		whole_driving();
		break;
	}
